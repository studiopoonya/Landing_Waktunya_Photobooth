...
import {
  ...
  LayoutDashboard, UserCog, Tag, Percent, Menu,
} from 'lucide-react';
import {
...
function PlaceholderPage({ title, icon: Icon, color }) {
  return (
    <div className="flex flex-col items-center justify-center h-96 text-center">
      <div className={`w-16 h-16 ${color} rounded-2xl flex items-center justify-center mb-4 opacity-20`}>
...
export default function AdminDashboard() {
    ...
    <div className={`flex flex-col h-full ${mobile ? '' : 'w-56'}`}>
      ...
      <div className="px-5 py-5 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-black text-slate-900 text-sm leading-tight">Waktunya</div>
            <div className="font-black text-slate-900 text-sm leading-tight">Photobooth</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-slate-100">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-all"
        >
          <LogOut size={17} />
          Logout
        </button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex">
      ...
      {/* Sidebar Desktop */}
      <aside className="hidden md:flex flex-col w-56 bg-white border-r border-slate-200 fixed left-0 top-0 h-full z-30">
        <Sidebar />
      </aside>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 h-full w-56 bg-white border-r border-slate-200 z-50 md:hidden"
            >
              <Sidebar mobile />
            </motion.aside>
          </>
      ...
      <div className="flex-1 md:ml-56 flex flex-col min-h-screen">
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        leads.data.map((lead, i) => {
                          ...
                          return (
                            <tr key={lead.id}
                              className={`border-b border-slate-100 transition-colors ${isSelected ? 'bg-sky-50/60' : 'hover:bg-slate-50'}`}
                            >
                ...
                {!loading && leads.last_page > 1 && (
                  <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={leads.current_page === 1}
                          ...
                          p === '...' ? (
                            ...
                            <button key={p} onClick={() => setPage(p)}
                              className={`w-8 h-8 rounded-lg text-sm font-semibold transition-colors ${
                                p === leads.current_page ? 'gradient-brand text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
                              }`}
                            >{p}</button>
                          )
                        )}
                      <button onClick={() => setPage(p => Math.min(leads.last_page, p + 1))} disabled={leads.current_page === leads.last_page}
                        className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
          ...
          {/* ── PLACEHOLDER PAGES ── */}
          {activePage === 'account' && <AccountPage showToast={showToast} />}
          {activePage === 'harga'   && <HargaPage showToast={showToast} />}
          {activePage === 'promo'   && <PromoPage showToast={showToast} />}
